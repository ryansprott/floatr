export default class Source {
  ...
  getMaxDistance() {
    return Math.max(
      ...this.getFilteredPositions().map((position) => position.distance)
    );
  }
  ...
  getAverageSpeed() {
    ...
    return averageSpeed > 1.0 ? Math.round(averageSpeed * 100) / 100 : "STOP";
  }

  getLastCourse() {
    const pos = this.getFilteredPositions()
      .filter((el) => el.course !== null)
      .pop();
