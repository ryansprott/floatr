...
export default class extends Controller {
  ...
  drawFirstAndLastMarkers() {
    this.drawMarker(this.positionData[0], "green");

    this.drawMarker(this.positionData[this.positionData.length - 1], "red");
  }

  drawMarker(position, fillColor) {
    getMarker(getLatLng(position), {
      scale: 0.1,
      fillColor: fillColor,
    }).setMap(this.map);
  }
  ...
  chunkedPolylines() {
    ...
    if (this.positionData.length > 1) {
      ...
      for (let i = 0; i < this.positionData.length - 1; i++) {
        ...
        [pair.startPosition(), pair.endPosition()].map((position) => {
      ...
      this.bounds.extend(getLatLng(this.positionData[0]));
    }
